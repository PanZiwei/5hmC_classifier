...
The script is to downsampling all samples with RandomUnderSampler -> random forest with GridSearch for best parameter
1. Downsampling all samples with idea ratio 0:1:2 = 14:14:1
2. Split the sample into training and testing datasets
3. GridSearch for all parameters to find the best parameter on the training dataset
...
if __name__ == '__main__':
    ## Create the parser #
    parser = argparse.ArgumentParser(description="GridSearch for Megalodon result")
    ...
    parser.add_argument('--pkl_model', required=False,
                        default='GridSearch.model.pkl',
                        help='model name for best parameter. default GridSearch.model.pkl'),
...
#                                     n_jobs=-1)
    ...
    _confusion_matrix.to_csv(os.path.join(output_path, "GridSearch.confusion_matrix.test.csv"), header = True, index= True, sep = ',', float_format='%.4f')
    
    plot_cm(y_test, y_pred, os.path.join(output_path, "GridSearch.confusion_matrix.test.png"), label = label, figsize=(10,10))
    
    ##Save the cv result 
    cv_table = pd.DataFrame(pipe_result.cv_results_)
    cv_table.to_csv(os.path.join(output_path, 'GridSearch.cv_result.csv'), sep='\t', index = True, header = True, float_format='%.4f')
    
    ## Save the classification report
    report = pd.DataFrame(classification_report(
        y_true, y_pred, target_names=label, output_dict=True)
                         ).transpose()
    report.to_csv(os.path.join(output_path, "GridSearch.class_report.test.csv"), header = True, index= True, sep = ',', float_format='%.4f')
