...
#10x2x5x2x2x2=800
n_estimators =  [int(x) for x in np.linspace(start = 50, stop = 300, num = 20)] # number of trees in the random forest
...
#assign the parameters to the named step in the pipeline. In your case classifier, should be the same in the imbpipeline step
random_grid = {'classifier__' + key: random_grid[key] for key in random_grid}
...
rf_random_search = RandomizedSearchCV(estimator = rf_pipeline, 
                                      ...
                                      n_iter = 30,  #The number of parameter settings that are tried 
                                      random_state=35,
                                      n_jobs=-1)
...
import joblib
joblib.dump(rf_random_search.best_estimator_, os.path.join(output_path,'rf.pkl'))

df_result = pd.DataFrame(rf_result.cv_results_)
df_result.to_csv(os.path.join(output_path, 'cv_result.tsv'), sep='\t', index = None)
...
cm_analysis(y_test, y_pred, os.path.join(output_path, 'confusion_matrix_testing.png'),
...
df.to_csv(os.path.join(output_path, 'class_report_testing.csv'), header = True, index= None, sep = ',', float_format='%.4f')
