...
output_path='/pod/2/li-lab/Ziwei/Nanopore/daily/2021-10-11'
...
max_depth = [10,25] # maximum number of levels allowed in each decision tree
min_samples_split = [10] # minimum sample number to split a node
min_samples_leaf = [2] # minimum sample number that can be stored in a leaf node
...
import joblib
joblib.dump(rf_random_search.best_estimator_, os.path.join(output_path,'rf_20.pkl'))

df_result = pd.DataFrame(rf_result.cv_results_)
df_result.to_csv(os.path.join(output_path, 'cv_result_20.tsv'), sep='\t', index = None)
...
cm_analysis(y_test, y_pred, os.path.join(output_path, 'confusion_matrix_20.png'),
...
df.to_csv(os.path.join(output_path, 'class_report_20.csv'), header = True, index= None, sep = ',', float_format='%.4f')
