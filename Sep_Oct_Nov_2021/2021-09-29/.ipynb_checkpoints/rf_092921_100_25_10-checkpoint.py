...
print('Mean f1_macro:{:.3f} on cv'.format(np.mean(f1_macro)))
print("Training is done!")

#Fitting the model
rf = rf_model.fit(X_train_new, y_train_new)

y_pred = rf_model.predict(X_test)
test_score = rf_model.score(X_test, y_test)
print('f1_macro:{:.3f} on test dataset'.format(test_score))
...
cm_analysis(y_test, y_pred, os.path.join(output_path, 'confusion_matrix_100_25_10.png'),
...
df.to_csv(os.path.join(output_path, 'class_report_100_25_10.csv'), header = True, index= None, sep = ',', float_format='%.4f')
