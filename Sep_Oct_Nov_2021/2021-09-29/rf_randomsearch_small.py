...
#Define parameter
#10x2x5x2x2x2=800
n_estimators =  [int(x) for x in np.linspace(start = 50, stop = 120, num = 10)] # number of trees in the random forest
max_features = ['auto', 'sqrt'] # number of features in consideration at every split
max_depth = [int(x) for x in np.linspace(start = 5, stop = 20, num = 5)] # maximum number of levels allowed in each decision tree
...
import joblib
joblib.dump(rf_random.best_estimator_, os.path.join(output_path,'rf_small.pkl'))

df_result = pd.DataFrame(rf_result.cv_results_)
df_result.to_csv(os.path.join(output_path, 'cv_small.tsv'), sep='\t', index = None)
...
cm_analysis(y_test, y_pred, os.path.join(output_path, 'confusion_matrix_small.png'),
...
df.to_csv(os.path.join(output_path, 'class_report_small.csv'), header = True, index= None, sep = ',', float_format='%.4f')
