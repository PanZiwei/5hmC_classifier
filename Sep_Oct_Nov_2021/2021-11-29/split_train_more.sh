#!/usr/bin/bash
###SLURM HEADER
#SBATCH --job-name=512000_train_model
#SBATCH --output=/fastscratch/c-panz/2021-11-29/log/train_model_512000.log
#SBATCH --err=/fastscratch/c-panz/2021-11-29/log/train_model_512000.err
...
output_path=/fastscratch/c-panz/2021-11-29/512000

####Check file lines: wc -l 
...
#### Use minimum requirement: 512*1000 samples = 512000 
...
#### 2668*192=512256
for i in {1..192};do cat $output_path/T4_5hmC.train.tsv >> $output_path/T4_5hmC.train.repeated.tsv; done
echo "T4 mimic is done!"

# 2. randomly select equally number of 5mC/5C samples ->  split samples for training/validating(9:1)
num_feature=516400 # 512256+296*14 (5hmC:5mC:5C = 1:14:14)
...
# 4. split samples for training and validating
head -512256 $output_path/lambda_5mC.tsv > $output_path/lambda_5mC.train.tsv
tail -4144 $output_path/lambda_5mC.tsv > $output_path/lambda_5mC.valid.tsv
head -512256 $output_path/lambda_5C.tsv > $output_path/lambda_5C.train.tsv
...
rm -rf lambda_5mC.train.tsv lambda_5C.train.tsv lambda_5mC.tsv
rm -rf lambda_5mC.valid.tsv lambda_5C.valid.tsv lambda_5C.tsv
