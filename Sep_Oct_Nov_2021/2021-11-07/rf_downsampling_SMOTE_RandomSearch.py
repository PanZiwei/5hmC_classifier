...
if __name__ == '__main__':
    ...
    parser.add_argument('--pkl_model', required=False,
                        default='RandomSearch.model.pkl',
                        help='model name for best parameter. default RandomSearch.model.pkl'),
...
#    stratified_kfold = StratifiedKFold(n_splits = 3, random_state = 42, shuffle = True)
    ...
    _confusion_matrix.to_csv(os.path.join(output_path, "RandomSearch.confusion_matrix.test.csv"), header = True, index= True, sep = ',', float_format='%.4f')
    
    plot_cm(y_test, y_pred, os.path.join(output_path, "RandomSearch.confusion_matrix.test.png"), label = label, figsize=(10,10))
    
    ##Save the cv result 
    cv_table = pd.DataFrame(pipe_result.cv_results_)
    cv_table.to_csv(os.path.join(output_path, 'RandomSearch.cv_result.csv'), sep='\t', index = True, header = True, float_format='%.4f')
    
    ## Save the classification report
    report = pd.DataFrame(classification_report(
        y_true, y_pred, target_names=label, output_dict=True)
                         ).transpose()
    report.to_csv(os.path.join(output_path, "RandomSearch.class_report.test.csv"), header = True, index= True, sep = ',', float_format='%.4f')
