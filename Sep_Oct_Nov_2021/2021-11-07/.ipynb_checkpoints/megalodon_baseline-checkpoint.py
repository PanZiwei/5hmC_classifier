...
The script is to evaluate Megalodon baseline performance on the whole dataset
...
if __name__ == '__main__':
    ...
    _confusion_matrix.to_csv(os.path.join(output_path, "Megalodon.max_value.csv"), header = True, index= True, sep = ',', float_format='%.4f')
    
    plot_cm(y_true, y_pred, os.path.join(output_path, "Megalodon.max_value.png"), label = label, figsize=(10,10))
    ...
    report.to_csv(os.path.join(output_path, "Megalodon.max_value.csv"), header = True, index= None, sep = ',', float_format='%.4f')
    del y_pred
    ...
    _confusion_matrix = confusion_matrix(y_true, y_pred)
    _confusion_matrix = pd.DataFrame(_confusion_matrix, index=label, columns=label)
    _confusion_matrix.to_csv(os.path.join(output_path, "Megalodon.zero_cutoff.csv"), header = True, index= True, sep = ',', float_format='%.4f')
    
    plot_cm(y_true, y_pred, os.path.join(output_path, "Megalodon.zero_cutoff.png"), label = label, figsize=(10,10))
    ...
    report.to_csv(os.path.join(output_path, "Megalodon.zero_cutoff.csv"), header = True, index= None, sep = ',', float_format='%.4f')
    del y_pred
