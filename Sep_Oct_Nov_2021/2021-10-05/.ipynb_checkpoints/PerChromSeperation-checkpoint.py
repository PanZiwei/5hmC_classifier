...
#            raise RuntimeError('Alignment not found.')
    ...
    T4.to_csv(os.path.join(output_path, 'T4_list.tsv'), sep=',',index = None, header=None)
    lambda_.to_csv(os.path.join(output_path, 'lambda_list.tsv'), sep=',',index = None, header=None)
    human.to_csv(os.path.join(output_path, 'human_list.tsv'), sep=',',index = None, header=None)
