...
rf_result = rf_random.fit(X_train_new, X_train_new)
...
import joblib
joblib.dump(rf_random.best_estimator_, os.path.join(output_path,'rf.pkl'))

df_result = pd.DataFrame(rf_result.cv_results_)
df_result.to_csv(os.path.join(output_path, 'cv.tsv'), sep='\t', index = None)
