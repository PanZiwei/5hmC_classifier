...
Downsampling all samples with idea ratio 0:1:2 = 1:1:1
...
if __name__ == '__main__':
    ...
    import joblib
    joblib.dump(pipe_search.best_estimator_, os.path.join(output_path,'rf.test.RandomSearch.best.pkl'))
    ...
    _confusion_matrix.to_csv(os.path.join(output_path, "rf.test.RandomSearch.confusion_matrix.csv"), header = True, index= True, sep = ',', float_format='%.4f')
    
    plot_cm(y_true, y_pred, os.path.join(output_path, "rf.test.RandomSearch.confusion_matrix.png"), label = label, figsize=(10,10))

    print('f1_macro for three-way maximum:',f1_score(y_true, y_pred, average='macro'))
    
     ##Save the cv result 
    cv_table = pd.DataFrame(pipe_result.cv_results_)
    cv_table.to_csv(os.path.join(output_path, 'rf.test.RandomSearch.cv_result.csv'), sep='\t', index = True, header = True, float_format='%.4f')
    
    ## Save the classification report
    report = pd.DataFrame(classification_report(
        y_true, y_pred, target_names=label, output_dict=True)
                         ).transpose()
    report.to_csv(os.path.join(output_path, "rf.test.RandomSearch.class_report.csv"), header = True, index= None, sep = ',', float_format='%.4f')
